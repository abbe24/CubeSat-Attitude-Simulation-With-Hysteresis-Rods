...
drawnow;
saveas(fig, 'Final_30Day_Mission_Dashboard11.png');
...
function dydt = myODE(t, y)
    % --- SAFETY LOCK 1: Hard Normalization ---
    % This prevents the 10^44 "explosion" by ensuring the solver 
    % always works with a valid unit quaternion.
    q_in = y(1:4);
    q_norm = norm(q_in);
    q = (q_in / q_norm)'; % Normalize and convert to row for quatmultiply
    
    % Angular velocity (column vector for dynamics)
    w = y(5:7); 
    ...
    rod_L = 0.095; 
    rod_r = 0.0005; 
    V_hyst = pi * rod_r^2 * rod_L; % Volume of 1 rod
    
    %Configuration	0 rods on X, 2 rods on Y, 2 rods on Z
    % Satellite Inertia (Standard 1U CubeSat)
    % (Using slightly different values allows for 3D nutation/wobble)
    %I = diag([0.0012, 0.0012, 0.0015]); 
    % Gerhardt CSSWE 3U CubeSat Inertia (kg*m^2)
    I_xx = 0.00551; 
    I_yy = 0.02552; 
    I_zz = 0.02565;
    % Define the matrix
    ...
    % 1. Environment: Constant Magnetic Field Test
    % Replace the propagator and field model with a constant vector
    %B_eci = [0; 0; 40e-6]; % Constant 40 microTesla along ECI Z-axis
    
    %pos = circularPropagator(t); % Uses your existing propagator
    pos = circularPropagatorGerhardt(t);
    B_eci = getEarthField(pos);  % Uses your existing dipole model
    R_bi = quat2rotm_manual(q); 
    %B_body = R_bi * B_eci;
    B_body = R_bi' * B_eci;
    
    % 2. Hysteresis Torque Calculation
    rod_axes = eye(3); 
    tau_hyst = zeros(3,1);
    ...
    for i = 1:3
        ...
        if num_rods > 0
            % Calculate B_hyst for this specific rod direction
            % This uses your existing calculate_B_hyst function
            B_rod = calculate_B_hyst(y, B_body, rod_dir);
        
            % Calculate total magnetic moment for this axis
            % m = (B * V_total) / mu0
            m_mag = (B_rod * (V_hyst * num_rods)) / mu_0;
            m_vec = m_mag * rod_dir;
        
            % Torque = m x B_external
            tau_hyst = tau_hyst + cross(m_vec, B_body);
        end
    end
    
    % 3. Kinematics: q_dot (Quaternion Rate)
    % q_dot = 0.5 * q_unit * [0, w]
    qdot_raw = 0.5 * quatmultiply(q, [0, w']);
    
    % Baumgarte Stabilization: Gently nudges the norm back to 1.0
    k_stabilize = 0.1; 
    norm_error = 1 - q_norm; % Note: used the original norm to calculate error
    qdot = qdot_raw + k_stabilize * norm_error * q;
    
    % --- 4. Dynamics: w_dot (Angular Acceleration) ---
    w_col = y(5:7); 
    
    % New: Define the Bar Magnet dipole (0.3 Am^2 aligned with Body X-axis)
    m_bar = [0.3; 0; 0]; 
    
    % New: Calculate Bar Magnet Torque (tau = m x B)
    tau_bar = cross(m_bar, B_body);
    
    % Updated: Include tau_bar in the Euler Equation
    % w_dot = I \ (tau_hyst + tau_bar - w x Iw)
    wdot = I \ (tau_hyst + tau_bar - cross(w_col, I * w_col));
    %wdot = I \ (tau_bar - cross(w_col, I * w_col));
    
    % 5. Pack Output as a Column Vector
    % qdot is 1x4 (from quatmultiply), so qdot' is 4x1
    % wdot is 3x1 (from matrix division)
    dydt = [qdot(:); wdot(:)];
end

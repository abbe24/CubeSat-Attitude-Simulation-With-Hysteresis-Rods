...
drawnow;
saveas(fig, 'Final_30Day_Mission_Dashboard4.1.png');
...
function dydt = myODE(t, y)
    ...
    tau_hyst = cross(m_hyst, B_body);

    %permanent magnet
    m_pm_val = 0.01; % Dipole strength in Ampere-meters^2
    % assume the magnet is aligned with the satellite's Z-axis (long axis)
    m_pm_body = [0; 0; m_pm_val];
    tau_pm = cross(m_pm_body, B_body);

    tau_total = tau_hyst + tau_pm;
